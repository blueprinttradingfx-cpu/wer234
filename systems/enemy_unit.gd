...
func _change_target_position() -> void:
	...
	print("[EnemyUnit] Re-routed to new position: %s" % target_position)
...
func _cloak() -> void:
	...
	print("[EnemyUnit] Cloaked for %.1f seconds" % cloak_duration)

func _uncloak() -> void:
	is_cloaked = false
	modulate = Color(1, 1, 1, 1.0)  # Restore full opacity
	print("[EnemyUnit] Uncloaked")
...
func _spawn_payload_enemy() -> void:
	...
	payload_spawned += 1
	print("[EnemyUnit] Spawned payload enemy %d/%d" % [payload_spawned, payload_capacity])
...
func _spawn_split_children() -> void:
	...
	print("[EnemyUnit] Split into %d children at depth %d" % [split_count, split_depth])
...
func _trigger_emp_pulse() -> void:
	...
	print("[EnemyUnit] EMP pulse triggered with radius %.1f and duration %.1f" % [emp_radius, emp_duration])
